...
public class EventResource {
    ...
    public Response subscribeToEvent(@PathParam("id") Long eventId) {
        ...
        try {
            ...
            LOG.info(
                "User " + currentUser.id + " subscribed to event " + eventId
            );
    ...
    public Response unsubscribeFromEvent(@PathParam("id") Long eventId) {
        ...
        User currentUser = userContext.getCurrentUser();
        Subscription subscription = Subscription.findByUserAndEvent(
            currentUser,
            event
        );
        ...
        try {
            subscription.delete();
            LOG.info(
                "User " + currentUser.id + " unsubscribed from event " + eventId
            );
            return Response.ok(
                Map.of("message", "Successfully unsubscribed from event")
            ).build();
        } catch (Exception e) {
    ...
    public Response getSubscriptionStatus(@PathParam("id") Long eventId) {
        ...
        response.put(
            ...
            event.maxParticipants != null &&
                totalSubscriptions >= event.maxParticipants
        );
    ...
    public Response getEventSubscriptions(
        ...
        try {
            ...
            if (searchTerm != null && !searchTerm.trim().isEmpty()) {
                subscriptions = Subscription.findByEventWithSearch(
                    event,
                    searchTerm,
                    page,
                    size
                );
                totalCount = Subscription.countByEventWithSearch(
                    event,
                    searchTerm
                );
            } else {
                subscriptions = Subscription.findByEventPaginated(
                    event,
                    page,
                    size
                );
            ...
            List<ParticipantDTO> participantDTOs = subscriptions
                .stream()
                .map(ParticipantDTO::new)
                .collect(Collectors.toList());

            PaginatedParticipantsDTO response = new PaginatedParticipantsDTO(
                participantDTOs,
                totalCount,
                page,
                size
    ...
    public Response getMySubscriptions() {
        try {
            User currentUser = userContext.getCurrentUser();
            List<Subscription> subscriptions = Subscription.findByUser(
                currentUser
            );
