...
public class CDNService {
    ...
    private final Map<String, Integer> userUploadCounts = new ConcurrentHashMap<>();
    private final Map<String, Long> userLastResetTime = new ConcurrentHashMap<>();

    private static CDNService instance;
    ...
    @Inject
    @ConfigProperty(name = "cdn.upload.allowed-types", defaultValue = "image/jpeg,image/png,image/webp,image/gif")
    ...
    public String uploadFile(FileUpload upload, String type, String userId) throws IOException {
        checkRateLimit(userId);

        String contentType = upload.contentType();
        if (!allowedTypes.contains(contentType)) {
            throw new WebApplicationException("Tipo de arquivo não permitido. Tipos permitidos: " + String.join(", ", allowedTypes), 
                    Response.Status.BAD_REQUEST);
        }

        if (upload.size() > maxFileSize) {
            throw new WebApplicationException("Arquivo muito grande. Tamanho máximo: " + maxFileSize / (1024 * 1024) + "MB", 
                    Response.Status.BAD_REQUEST);
        }
        ...
        if ("avatar".equalsIgnoreCase(type)) {
            ...
            throw new WebApplicationException("Tipo de arquivo inválido. Use 'avatar' ou 'honor'", 
                    Response.Status.BAD_REQUEST);
        }
        ...
        if ("avatar".equalsIgnoreCase(type)) {
            processAvatarImage(upload.uploadedFile(), targetPath.toString(), extension);
            for (int size : avatarSizes) {
                resizeImage(targetPath.toString(), 
                        Paths.get(uploadDirectory, subDir, size + "_" + safeFileName).toString(), 
                        size, size, extension);
            }
        } else {
            optimizeImage(upload.uploadedFile(), targetPath.toString(), extension);
        }
        
        return safeFileName;
    }

    public String downloadImageFromUrl(String imageUrl, String type, String userId) throws IOException {
        checkRateLimit(userId);
        
        if (imageUrl == null || imageUrl.isEmpty()) {
            throw new WebApplicationException("URL da imagem é obrigatória", 
                    Response.Status.BAD_REQUEST);
        }
        ...
        if (responseCode != HttpURLConnection.HTTP_OK) {
            throw new WebApplicationException("Falha ao baixar imagem da URL. Código de resposta: " + responseCode, 
                    Response.Status.BAD_REQUEST);
        }
        ...
        try (InputStream inputStream = connection.getInputStream()) {
            ...
            if (originalImage == null) {
                throw new WebApplicationException("Não foi possível ler a imagem da URL fornecida", 
                        Response.Status.BAD_REQUEST);
            }
            
            if ("avatar".equalsIgnoreCase(type)) {
                if (originalImage.getWidth() > maxAvatarWidth || originalImage.getHeight() > maxAvatarHeight) {
                    BufferedImage resizedImage = resizeImageProportionally(originalImage, maxAvatarWidth, maxAvatarHeight);
    ...
    private void processAvatarImage(Path sourcePath, String targetPath, String extension) throws IOException {
    ...
    private void optimizeImage(Path sourcePath, String targetPath, String extension) throws IOException {
        BufferedImage originalImage = ImageIO.read(sourcePath.toFile());

        saveImage(originalImage, targetPath, extension);
    }

    private void resizeImage(String sourcePath, String targetPath, int width, int height, String extension) throws IOException {
    ...
    private BufferedImage resizeImageProportionally(BufferedImage original, int maxWidth, int maxHeight) {
    ...
    private void saveImage(BufferedImage image, String path, String extension) throws IOException {
    ...
    public Path getFilePath(String filename, String type) {
        if (filename == null || filename.isEmpty() || filename.contains("..") || filename.contains("/")) {
            throw new WebApplicationException("Nome de arquivo inválido", Response.Status.BAD_REQUEST);
        }
        ...
        if (!Files.exists(filePath)) {
            throw new WebApplicationException("Arquivo não encontrado", Response.Status.NOT_FOUND);
        }
    ...
    public void deleteFile(String filename, String type) throws IOException {
        ...
        if ("avatar".equalsIgnoreCase(type)) {
            for (int size : avatarSizes) {
                Path sizedPath = Paths.get(uploadDirectory, "avatars", size + "_" + filename);
    ...
    private void checkRateLimit(String userId) {
        ...
        if (currentCount >= maxRequestsPerWindow) {
            throw new WebApplicationException("Limite de uploads excedido. Tente novamente mais tarde.", 
