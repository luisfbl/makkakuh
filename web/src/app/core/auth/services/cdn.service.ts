...
export class CDNService {
  ...
  /**
   * Faz o upload de uma imagem para o CDN
   * @param file Arquivo a ser enviado
   * @param type Tipo do arquivo (avatar ou honor)
  ...
  /**
   * Obtém a URL completa para uma imagem no CDN
   * @param filename Nome do arquivo
   * @param type Tipo do arquivo (avatar ou honor)
   * @param size Tamanho da imagem (apenas para avatares)
  ...
  getImageUrl(filename: string | null | undefined, type: 'avatar' | 'honor', size?: number): string {
    ...
    // Adicionar parâmetro de tamanho apenas para avatares
    if (type === 'avatar' && size) {
      url += `?size=${size}`;
    }
  ...
  /**
   * Obtém a URL para a imagem padrão
  ...
  deleteImage(filename: string, type: 'avatar' | 'honor'): Observable<{ success: boolean, message: string }> {
    return this.http.delete<{ success: boolean, message: string }>(`${this.apiUrl}/cdn/images/${type}/${filename}`).pipe(
      catchError(error => {
